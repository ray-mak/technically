...
export async function POST(req: NextRequest) {
  try {
    ...
    const qnaPairs = interview.questions
      .map(
        (q, i) =>
    ...
    return new Response(
      JSON.stringify({ questions: interview.questions, feedback }),
      { status: 200 }
    );
  } catch (error) {
